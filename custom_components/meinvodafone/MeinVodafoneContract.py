...
class MeinVodafoneContract:
    ...
    @property
    def plan_name(self) -> str | None: return self.usage_data.get(PLAN_NAME)
    @property
    ...
    def data_available_until(self) -> datetime.date | None:
        """Return end date as date object for HA."""
    ...
    @property
    def is_data_available_until_supported(self) -> bool: return self.data_available_until is not None
    ...
    def billing_cycle_days(self) -> int | None:
        """Days until next cycle start (API 1)."""
        target = self.tariff_data.get(NEXT_CYCLE_START)
        if not target:
            return None
        try:
            now = datetime.datetime.now(datetime.timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
            target_date = datetime.datetime.strptime(str(target)[:10], ISO_DATE_FORMAT).replace(tzinfo=datetime.timezone.utc)
            return max(0, (target_date - now).days)
        except Exception:
            return None

    @property
    def is_billing_cycle_days_supported(self) -> bool: return self.billing_cycle_days is not None