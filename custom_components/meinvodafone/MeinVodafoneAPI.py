...
class MeinVodafoneAPI:
    ...
    async def get_contracts(self) -> list[str]:
        """Retrieve list of mobile contracts."""
        contracts = []
    ...
    async def get_tariff_details(self, contract_number: str) -> dict[str, Any]:
        """Fetch plan name, price and billing cycle from URL 1."""
        tariff_data = {PLAN_NAME: None, PLAN_PRICE: None, CYCLE_END: None}
        timestamp = f"{int(time.time())}"
        try:
            ...
            async with self.session.get(url, headers=headers, timeout=API_TIMEOUT) as resp:
                if resp.status == 200:
                    ...
                    tariff_data[PLAN_NAME] = details.get("name")
                    tariff_data[CYCLE_END] = details.get("cycleEndDate")
            ...
            _LOGGER.error("Error fetching tariff details")
        return tariff_data

    async def get_contract_usage(self, contract_number: str) -> dict[str, Any]:
        """Fetch detailed usage data from URL 2."""
        usage_data = {BILLING: {}, MINUTES: [], SMS: [], DATA: [], DATA_VALID_UNTIL: None}
        timestamp = f"{int(time.time())}"
        try:
            ...
            _LOGGER.error("Usage fetch failed: %s", error)
